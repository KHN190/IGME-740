...
void main() {
  ...
  float noiseRate = 2.5f;
  float speed = 1.5f;
  noise = (cnoise(vertex_normal*noiseRate + vec3(time)*speed) + 0.866) / (2* 0.866); 
  float displacement = noise * 3.0f;

  // translate the position along the normal

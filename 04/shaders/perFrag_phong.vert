...
void main() {
  ...
  // use per fragment normal
  vec3 normal = vec3(viewMat * modelMat * vec4(vertex_normal, 0.0));

  float spec0 = spec(viewPos, lightPos0, vertex_position, normal);
  float spec1 = spec(viewPos, lightPos1, vertex_position, normal);
  float lambert0 = lambert(lightPos0, vertex_position, normal);
  float lambert1 = lambert(lightPos1, vertex_position, normal);

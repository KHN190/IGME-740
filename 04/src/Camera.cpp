...
void Camera::PrintProperty() {
  ...
  // "<<frustum.ftl.x<<", "<<frustum.ftl.y<<", "<<frustum.ftl.z<<endl; cout<<"ftr
  // = "<<frustum.ftr.x<<", "<<frustum.ftr.y<<", "<<frustum.ftr.z<<endl;
}

void Camera::set(float eye_x, float eye_y, float eye_z, float lookat_x,
...
void Camera::mouseClick(int button, int state, int x, int y, int winW,
                        int winH) {
  if (x >= winW || y >= winH) {
    return;
  }
...
void Camera ::CameraPan() {
  eye.x += M_PAN_PAR * (viewMat[0][0] * (mouse_pre_pos.x - mouse_pos.x) +
                        viewMat[0][1] * (mouse_pre_pos.y - mouse_pos.y));
  eye.y += M_PAN_PAR * (viewMat[1][0] * (mouse_pre_pos.x - mouse_pos.x) +
                        viewMat[1][1] * (mouse_pre_pos.y - mouse_pos.y));
  eye.z += M_PAN_PAR * (viewMat[2][0] * (mouse_pre_pos.x - mouse_pos.x) +
                        viewMat[2][1] * (mouse_pre_pos.y - mouse_pos.y));

  lookat.x += M_PAN_PAR * (viewMat[0][0] * (mouse_pre_pos.x - mouse_pos.x) +
                           viewMat[0][1] * (mouse_pre_pos.y - mouse_pos.y));
  lookat.y += M_PAN_PAR * (viewMat[1][0] * (mouse_pre_pos.x - mouse_pos.x) +
                           viewMat[1][1] * (mouse_pre_pos.y - mouse_pos.y));
  lookat.z += M_PAN_PAR * (viewMat[2][0] * (mouse_pre_pos.x - mouse_pos.x) +
                           viewMat[2][1] * (mouse_pre_pos.y - mouse_pos.y));
}

void Camera ::CameraPan_fp() {
  eye.x += M_PAN_PAR_FP * (viewMat[0][0] * (key_pos.x - key_pre_pos.x) +
                           viewMat[0][2] * (key_pos.y - key_pre_pos.y));
  eye.y += M_PAN_PAR_FP * (viewMat[1][0] * (key_pos.x - key_pre_pos.x) +
                           viewMat[1][2] * (key_pos.y - key_pre_pos.y));
  eye.z += M_PAN_PAR_FP * (viewMat[2][0] * (key_pos.x - key_pre_pos.x) +
                           viewMat[2][2] * (key_pos.y - key_pre_pos.y));

  lookat.x += M_PAN_PAR_FP * (viewMat[0][0] * (key_pos.x - key_pre_pos.x) +
                              viewMat[0][2] * (key_pos.y - key_pre_pos.y));
  lookat.y += M_PAN_PAR_FP * (viewMat[1][0] * (key_pos.x - key_pre_pos.x) +
                              viewMat[1][2] * (key_pos.y - key_pre_pos.y));
  lookat.z += M_PAN_PAR_FP * (viewMat[2][0] * (key_pos.x - key_pre_pos.x) +
                              viewMat[2][2] * (key_pos.y - key_pre_pos.y));
}
...
void Camera::CameraRotate_fp(int winW, int winH) {
  float nh =
      2.0f * near_plane *
      (float)tan(fovy * pi<float>() / 180.0f * 0.5f); // near-plane height
  float len = length(eye - lookat);

  vec3 t;
  t.x = nh * M_ROTATE_PAR_FP * (mouse_pos.x - mouse_pre_pos.x) * len /
        (float)winH / near_plane;
  t.y = nh * M_ROTATE_PAR_FP * (mouse_pos.y - mouse_pre_pos.y) * len /
        (float)winH / near_plane;
  t.z = -len;
  t = normalize(t);
  t *= len;

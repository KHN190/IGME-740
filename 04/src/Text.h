...
public:
  ...
  void draw(const float x, const float y, char *textContent, const int winW,
            const int winH, const int characterSpace = 3);
};
...
inline void Text::setFont(void *f) { font = f; }
...
inline void Text::draw(const float x, const float y, char *textContent,
  ...
  glTranslatef(0, (float)-winH, 0); // move the origin from the bottom left

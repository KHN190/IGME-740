...
void ShaderClass::createFromContent(const char *sourceContent,
                                    GLenum targetType) {
  ...
  glGetShaderiv(id, GL_COMPILE_STATUS, &status);
  if (status != GL_TRUE) {
    cout << "Error: The shader is created but incorrect, see the shader info "
            "log below!"
         << endl;
    printShaderInfoLog(id);
  }
...
char *ShaderClass::loadShaderFile(const char *fn) {
  ...
  /*
     ...
     + 1)); count = fread(content, sizeof(char), count, fp); content[count] =
     '\0';
                          }
                          fclose(fp);
                  }

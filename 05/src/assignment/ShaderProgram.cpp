...
#include <iostream>
using namespace std;
ShaderProgram::ShaderProgram(void) { id = 0; }

ShaderProgram::~ShaderProgram(void) {}

void ShaderProgram::create() { id = glCreateProgram(); }
...
//			glVertexAttrib4fARB(i, floatPtr[(i - loc) * 4 + 0],
//floatPtr[(i - loc) * 4 + 1], floatPtr[(i - loc) * 4 + 2], floatPtr[(i - loc) *
//4 + 3]);
//		}
//	}
//	//else
...
void ShaderProgram::setFloat1V(const char *name, unsigned int count,
...
void ShaderProgram::setFloat3V(const char *name, unsigned int count,
                               const float *floatPtr) {
  unsigned int loc = glGetUniformLocation(id, name);
  if (loc == -1) {
...
void ShaderProgram::setMatrix4fv(const char *name, unsigned int count,
                                 const float *floatPtr) {
  unsigned int loc = glGetUniformLocation(id, name);
  if (loc == -1) {
...
void ShaderProgram::createDataTexture(unsigned int &pGLTexID, float *pData,
  ...
  // glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA_FLOAT32_ATI/*GL_RGBA32F_ARB*/,
  // pMaxWidth, pMaxHeight, 0, GL_RGBA, GL_FLOAT, pData );

  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F /*GL_RGBA32F_ARB*/, pMaxWidth,
               pMaxHeight, 0, GL_RGBA, GL_FLOAT, pData);
